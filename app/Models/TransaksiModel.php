...
class TransaksiModel extends Model
{
    ...
    public function getSaldoNasabah(string $idNasabah,bool $isAdmin): string
    {
        $this->db->transBegin();
        ...
        $saldo = $this->db->table('dompet')->select("uang")->where('id_user',$idNasabah)->get()->getResultArray();
    ...
    public function tarikSaldo(array $data): array
    {
        try {
            ...
            if ($pemilik == "bst") {
                ...
                $this->db->query("INSERT INTO tarik_saldo (id_transaksi,jumlah_tarik,description) VALUES('$idtransaksi',$jumlahTarik,'$description')");
                $this->db->query("UPDATE dompet SET uang=uang-$jumlahTarik WHERE id_user IS NULL;");
            } 
            else {
                $this->db->query("INSERT INTO tarik_saldo (id_transaksi,jumlah_tarik) VALUES('$idtransaksi',$jumlahTarik)");
                $this->db->query("UPDATE dompet SET uang=uang-$jumlahTarik WHERE id_user='$idnasabah';");
            }
        ...
        try {
            if (isset($get['id_transaksi']) && !isset($get['idnasabah'])) {
                ...
                if ($code_transaksi == 'TSS') {
                    ...
                    $transaction  = $this->db->query("SELECT transaksi.id AS id_transaksi,transaksi.id_user,transaksi.jenis_transaksi,users.privilege,users.nama_lengkap,tarik_saldo.jumlah_tarik,tarik_saldo.description,transaksi.date 
                    FROM transaksi 
                    JOIN users ON (transaksi.id_user = users.id) 
        ...
        try {
            ...
            if (isset($get['date']) || isset($get['start']) && isset($get['end'])) {
                ...
                if ($get['jenis'] == 'penarikan-saldo') {
                    ...
                    $dataTts = $this->db->query("SELECT transaksi.id AS id_transaksi,transaksi.id_user,transaksi.jenis_transaksi,users.nama_lengkap,tarik_saldo.jumlah_tarik,transaksi.date 
                    FROM transaksi 
                    JOIN users        ON (transaksi.id_user = users.id)
                    JOIN tarik_saldo  ON (transaksi.id = tarik_saldo.id_transaksi) 
                    JOIN wilayah      ON (wilayah.id_user = users.id)
    ...
    public function updateTblSetorSampah()
    {
        ...
        $dataSampah  = $this->db->table('sampah')->select("id,harga,harga_pusat")->get()->getResultArray();
        $start = (int)strtotime('01-09-2022 00:01');
        $end = (int)strtotime('31-09-2022 23.59');

        foreach ($dataSampah as $value) {
            $idSampah = $value['id'];
            $harga = (int)$value['harga'];
            $hargaPusat = (int)$value['harga_pusat'];

            $this->db->query("UPDATE setor_sampah JOIN transaksi ON transaksi.id = setor_sampah.id_transaksi SET setor_sampah.harga=$harga,setor_sampah.harga_pusat=$hargaPusat WHERE setor_sampah.id_sampah = '$idSampah' AND transaksi.date BETWEEN '$start' AND '$end';");            
        }

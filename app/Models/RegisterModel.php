...
class RegisterModel extends Model
{
    ...
    // GET Last nasabah
    public function getLastNasabah(string $data): array
    {
        try {
            $lastNasabah = $this->db->table($this->table)->select('id')->like('id', $data, 'after')->limit(1)->orderBy('id','DESC')->get()->getResultArray();
            // $lastNasabah = $this->db->table($this->table)->select('id')->like('id', $data)->limit(1)->orderBy('created_at','DESC')->get()->getResultArray();
